...
from pyspark.sql.functions import sha2, concat_ws, coalesce, col, lit, to_timestamp
...
class DIFProcessedTableHelper:
    ...
    def dropProcessedTables(self):
        ...
        #         if self.aInstructions["Merge"] == False:
        #             self.executeSQL(f"DROP TABLE IF EXISTS {baseTable + G_TempSuffix}")
    ...
    def updateTempRecordsToTTable(self, pTimeStamp):
        deleteMode = self.aInstructions["Delete"]
        isMerge = self.aInstructions["Merge"]

        if deleteMode == "All":
            self.aLogger.info("DeleteMode: All. Truncating and recreating table.")
            self.dropProcessedTables()
            self.createProcessedTables()
            df = self.getAdjustedRawDataFrame(self.getRawDataFrame(pTimeStamp))
            df.write.mode("append").saveAsTable(self.aAsset[f'{AssetColumns.Processed_Table_Name}'])
            return

        if isMerge:
            self.mergeAndTrackChangesWithDelta(pTimeStamp)
        else:
            self.appendAndTrackChangesWithoutMerge(pTimeStamp)

    # def appendAndTrackChangesWithoutMerge(self, pTimeStamp):
    #     try:
    ...
    #
    #         dfIncoming = self.getAdjustedRawDataFrame(self.getRawDataFrame(pTimeStamp))
    ...
    #
    #         if self.aInstructions["CTable"] and not dfBefore.rdd.isEmpty():
    ...
    # def mergeAndTrackChangesWithDelta(self, pTimeStamp):
    #     try:
    ...
    #
    #         dfIncoming = self.getAdjustedRawDataFrame(self.getRawDataFrame(pTimeStamp))
    #         deltaTable = DeltaTable.forName(self.aSpark, mainTable)
    #         dfTarget = self.aSpark.table(mainTable).filter("CDC_LOAD_CODE != 'D'")
    #
    #         # BEFORE IMAGE - changed keys with different checksum
    #         dfBefore = dfIncoming.alias("source").join(
    #             dfTarget.alias("target"), on="KEY_CHECKSUM_TXT", how="inner"
    #         ).filter("target.NON_KEY_CHECKSUM_TXT != source.NON_KEY_CHECKSUM_TXT")
    #
    #         if self.aInstructions["CTable"] and not dfBefore.rdd.isEmpty():
    #             cTable = mainTable.replace(".T_", ".C_")
    ...
    #             dfBefore.write.mode("append").saveAsTable(cTable)
    #
    #         deleteFlagField, deleteFlagValue = self.getDeleteFieldAndValue() if self.aInstructions[
    #                                                                                 "Delete"] == "DeleteFlag" else (
    #         None, None)
    #
    #         if self.aInstructions["Delete"] == "Soft.Segment":
    ...
    #             formatted = ','.join(f"'{val}'" for val in snapshotValues)
    #             deltaTable.alias("target").merge(
    #                 dfIncoming.alias("source"),
    #                 "target.KEY_CHECKSUM_TXT = source.KEY_CHECKSUM_TXT AND target.CDC_LOAD_CODE != 'D'"
    #             ).whenMatchedUpdate(
    #                 condition=f"target.{snapshotField} IN ({formatted}) AND target.NON_KEY_CHECKSUM_TXT != source.NON_KEY_CHECKSUM_TXT",
    #                 set={col: f"source.{col}" for col in dfIncoming.columns if col != "KEY_CHECKSUM_TXT"} | {
    #                     "CDC_LOAD_CODE": "'U'"}
    #             ).whenNotMatchedInsert(
    #                 values={col: f"source.{col}" for col in dfIncoming.columns} | {"CDC_LOAD_CODE": "'I'"}
    #             ).whenNotMatchedBySourceUpdate(
    #                 condition=f"target.{snapshotField} IN ({formatted})",
    #                 set={"CDC_LOAD_CODE": "'D'", "LOAD_TS": f"CAST('{pTimeStamp} EST' AS TIMESTAMP)"} | (
    #                     {"END_TS": f"'{businessDate}'"} if self.aInstructions["FromToColumns"] else {})
    #             ).execute()
    #
    ...
    #                 "target.KEY_CHECKSUM_TXT = source.KEY_CHECKSUM_TXT AND target.CDC_LOAD_CODE != 'D'"
    #             ).whenMatchedUpdate(
    #                 condition="target.NON_KEY_CHECKSUM_TXT != source.NON_KEY_CHECKSUM_TXT",
    #                 set={col: f"source.{col}" for col in dfIncoming.columns if col != "KEY_CHECKSUM_TXT"} | {
    #                     "CDC_LOAD_CODE": "'U'"}
    #             ).whenNotMatchedInsert(
    #                 values={col: f"source.{col}" for col in dfIncoming.columns} | {"CDC_LOAD_CODE": "'I'"}
    #             ).whenNotMatchedBySourceUpdate(
    #                 condition="target.CDC_LOAD_CODE != 'D'",
    #                 set={"CDC_LOAD_CODE": "'D'", "LOAD_TS": f"CAST('{pTimeStamp} EST' AS TIMESTAMP)"} | (
    #                     {"END_TS": f"'{businessDate}'"} if self.aInstructions["FromToColumns"] else {})
    #             ).execute()
    #
    ...
    #                 "target.KEY_CHECKSUM_TXT = source.KEY_CHECKSUM_TXT AND target.CDC_LOAD_CODE != 'D'"
    #             ).whenMatchedUpdate(
    #                 condition=f"source.{deleteFlagField} = '{deleteFlagValue}'",
    #                 set={"CDC_LOAD_CODE": "'D'", "LOAD_TS": f"CAST('{pTimeStamp} EST' AS TIMESTAMP)"} | (
    #                     {"END_TS": f"'{businessDate}'"} if self.aInstructions["FromToColumns"] else {})
    #             ).whenMatchedUpdate(
    #                 condition=f"source.{deleteFlagField} != '{deleteFlagValue}' AND target.NON_KEY_CHECKSUM_TXT != source.NON_KEY_CHECKSUM_TXT",
    #                 set={col: f"source.{col}" for col in dfIncoming.columns if col != "KEY_CHECKSUM_TXT"} | {
    #                     "CDC_LOAD_CODE": "'U'"}
    #             ).whenNotMatchedInsert(
    #                 values={col: f"source.{col}" for col in dfIncoming.columns} | {"CDC_LOAD_CODE": "'I'"}
    ...
    #
    #         if self.aInstructions["CTable"]:
    #             dfAfter = deltaTable.toDF().filter(
    #                 (col("LOAD_TS") == to_timestamp(lit(pTimeStamp), 'yyyy-MM-dd HH:mm:ss')) &
    #                 (col("CDC_LOAD_CODE").isin("I", "U", "D"))
    #             )
    #             cTable = mainTable.replace(".T_", ".C_")
    #             dfAfter.write.mode("append").saveAsTable(cTable)
    #
    ...
    #         raise Exception(ex)
        ...
        # def mergeAndTrackChangesWithDelta(self, pTimeStamp):   ##All in One
        #     try:
        ...
        #                 dfBeforeImage = dfBeforeImage.withColumn("LOAD_TS", to_timestamp(lit(pTimeStamp_est),
        #                                                                                  'yyyy-MM-dd HH:mm:ss'))
        #
    ...
    def mergeAndTrackChangesWithDelta(self, pTimeStamp):
        try:
            pTimeStamp_utc = to_timestamp(lit(pTimeStamp))
            pTimeStamp_est = date_format(pTimeStamp_utc.cast(TimestampType()) + expr("INTERVAL 5 HOURS"), "yyyy-MM-dd HH:mm:ss")
            ...
            if self.aInstructions["Merge"]:
                ...
                dUpdateColumnsSourceDeleted = {col: f"dfSource.{col}" for col in dfSource.columns if col != 'KEY_CHECKSUM_TXT'}
                dUpdateColumnsSourceDeleted['CDC_LOAD_CODE'] = "'D'"
                ...
                if self.aInstructions["Delete"] == "None":
                    ...
                    for dField in self.aAsset["Fields"]:
                        if dField[f'{FieldColumns.Action_Flags_String}'] != '':
                            sDeleteFlag = dField[f'{FieldColumns.Target_Field_Name}'] + ' = "{}"'.format(dField[f'{FieldColumns.Action_Flags_String}'])
                            sNegateDeleteFlag = dField[f'{FieldColumns.Target_Field_Name}'] + ' != "{}"'.format(dField[f'{FieldColumns.Action_Flags_String}'])
                            break
                    deltaTable.alias("dfTarget").merge(
                        dfSource.alias("dfSource"),
                        "dfTarget.KEY_CHECKSUM_TXT = dfSource.KEY_CHECKSUM_TXT and dfTarget.CDC_LOAD_CODE != 'D'"
    ...
    def saveBeforeImageToCTable(self, dfSource, pTimeStamp_est):
        try:
            ...
            if self.aInstructions["Delete"] == "DeleteFlag":
                for dField in self.aAsset["Fields"]:
                    if dField['Action_Flags_String'] != '':
                        dfSource = dfSource.filter(col(dField[f'{FieldColumns.Target_Field_Name}']) != dField[f'{FieldColumns.Action_Flags_String}'])
                        break
            ...
            dfBeforeImage = dfBeforeImage.filter(col('CDC_LOAD_CODE').isin('I', 'U'))
            dfBeforeImage = dfBeforeImage.withColumn("CDC_LOAD_CODE", lit("X"))
            dfBeforeImage = dfBeforeImage.withColumn("LOAD_TS", to_timestamp(lit(pTimeStamp_est), 'yyyy-MM-dd HH:mm:ss'))
