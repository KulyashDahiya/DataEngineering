from pyspark.sql.functions import concat, col, lit, upper,lower, sha2, concat_ws, hash, to_date, date_format, regexp_replace, udf, substring, round 
...
class DIFProcessedTableHelper:
    ...
    def dropProcessedTables(self):
        try:
            ...
            sSQL = G_SQL.replace("<TableName>", self.aAsset[f'{AssetColumns.Processed_Table_Name}'].replace(".T_",".C_"))
            self.executeSQL(sSQL)
    ...
    def createOrReplaceViews(self):
        try:
            ...
            if self.aInstructions["LatestView"]:
                ...
                self.executeSQL(sSQL)
                self.aLogger.info("createOrReplaceViews.V_View.Created")
            else:
                self.aLogger.info("createOrReplaceViews.V_View.Skipped")
            ...
            if self.aInstructions["CTable"] and createDecryptionView:
                ...
                """
                self.executeSQL(sSQL)
                self.aLogger.info("createOrReplaceViews.CV_View.Created")
    ...
    def populateTempTable(self,pTimeStamp):
        try:
            ...
            try:
                ...
                dResults =sModule(self.aAsset, self.aAssetGroupConfig, self.aLogger).rawToProcessed(self.aAssetGroupConfig,self.aLogger,self.aAsset,dfRaw) 
            ...
            dfAdjusted.write.mode("append").saveAsTable(self.aAsset[f'{AssetColumns.Processed_Table_Name}']+G_TempSuffix)
    ...
    # Takes Raw DAtaFrame, applies FieldConversionRules and adds columns as needed
    # VERSION_ID
    ...
    def getNextVersionID(self):
        try:
            ...
            df = self.aSpark.sql("select max(VERSION_ID) as maxversion from " + self.aAsset[f'{AssetColumns.Processed_Table_Name}'])
            x = df.collect()[0][0]
            if x == None:
                x = 0
            x = x + 1
    ...
    def getColumns(self):
        ...
        for dField in sorted_fields:
            ...
            sColumnList = sColumnList + dField[f'{FieldColumns.Target_Field_Name}'] + " " + dField[f'{FieldColumns.Target_Data_Type_Code}']
        ...
        try:
            ...
            for dField in self.aAsset["Fields"]:
                if 'Encrypt' in dField.get(f'{FieldColumns.Data_Protection_Code}', ''):
                    return True
            return False
            self.aLogger.debug("shouldCreateDecryptionView.End")
        except Exception as ex:
